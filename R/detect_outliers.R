...
#' observation more than 1.5x the interquartile range will be tagged. The `clip`
#' option controls whether values outside this range will be trimmed to the
#' appropriate max or min.
#'
#' @param df The data.frame to be transformed
#' @param col The column of interest
#' @param threshold Values more than \code{threshold} times the interquartile
#'   range outside the interquartile range are considered outliers.
#' @param clip If \code{TRUE}, values above or below the specified threshold
#'   will be clipped to the relevant threshold.
#' @param flag_both_directions If \code{TRUE}, two boolean columns will be added
#'   to indicate whether the values was above or below the detection threshold.
#' @export
detect_outliers <- function(df, col, threshold = 1.5, clip = FALSE,
                            flag_both_directions = FALSE) {
  # Input checking
  ...
  iqr <- IQR(df[[col]])
  threshold_lower <- q_25 - (threshold * iqr)
  threshold_upper <- q_75 + (threshold * iqr)
  # Identify outliers
