...
test_that("internal params are calculated correctly", {
  ...
  clipped_df <- detect_outliers(df, "Sepal.Width", threshold = 1.5, clip = TRUE)
...
test_that("clipping works correctly", {
  clipped_df <- detect_outliers(df, "Sepal.Width", clip = TRUE)
...
test_that("bidirectional outlier flag works", {
  clipped_df <- detect_outliers(df, "Sepal.Length", clip = TRUE, flag_both_directions = T)
